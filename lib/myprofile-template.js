...
module.exports = {
    ...
    CONTENT: function(data) {
        ...
        html += makeCategoryTitle(data.activate.exp_global, "해외 경험");
        ...
        html += makeCategoryTitle(data.activate.exp_others, "기타 경험");
...
function makeContact(data) {
    var html = ``;
    if(data.user_info != undefined) {
        var phone = sanitizeHtml(data.user_info.phone);
...
function makeExpChart(data) {
    var html = ``;

    var date_value = {};
    if(data.high_school !== undefined) {
        date_value.high_school = [];
        date_value.high_school[0] = data.high_school.start_date;
        date_value.high_school[1] = data.high_school.end_date;
    }
    if(data.university !== undefined) {
        date_value.university = [];
        date_value.university[0] = data.university.start_date;
        date_value.university[1] = data.university.end_date;
    }
    if(data.job_exp !== undefined) {
        date_value.job_exp = [];
        for(var i=0; i<data.job_exp.length; i++) {
            date_value.job_exp[i] = [];
            date_value.job_exp[i][0] = data.job_exp[i].start_date;
            date_value.job_exp[i][1] = data.job_exp[i].end_date;
        }
    }
    if(data.global_exp !== undefined) {
        date_value.global_exp = [];
        for(var i=0; i<data.global_exp.length; i++) {
            date_value.global_exp[i] = [];
            date_value.global_exp[i][0] = data.global_exp[i].start_date;
            date_value.global_exp[i][1] = data.global_exp[i].end_date;
        }
    }
    if(data.other_exp !== undefined) {
        date_value.other_exp = [];
        for(var i=0; i<data.other_exp.length; i++) {
            date_value.other_exp[i] = [];
            date_value.other_exp[i][0] = data.other_exp[i].start_date;
            date_value.other_exp[i][1] = data.other_exp[i].end_date;
        }
    }
    
    if(Object.keys(date_value).length > 0) {
        date_value = JSON.stringify(date_value);
        date_value = date_value.replaceAll('\"','\'');
        date_value = date_value.replaceAll('년 ','-');
        date_value = date_value.replaceAll('월','');

        html += `
        <h3>경험 그래프</h3>
        <div>
            <canvas id="exp-chart"></canvas>
            <input id="exp-chart-value" type="hidden" value="${date_value}">
        </div>
        `;
    }
...
function makeMyChart(data) {
    ...
    var cnt_value = [];    // 직무경험, 해외경험, 자격증, 수상경험, 포트폴리오/작품, 논문/출판/특허
    if(data.job_exp !== undefined) { cnt_value[0] = data.job_exp.length; }
    else { cnt_value[0] = 0; }

    if(data.global_exp !== undefined) { cnt_value[1] = data.global_exp.length; }
    else { cnt_value[1] = 0; }

    if(data.certificate !== undefined) { cnt_value[2] = data.certificate.length; }
    else { cnt_value[2] = 0; }

    if(data.prize !== undefined) { cnt_value[3] = data.prize.length; }
    else { cnt_value[3] = 0; }

    if(data.port_work !== undefined) { cnt_value[4] = data.port_work.length; }
    else { cnt_value[4] = 0; }

    if(data.port_rights !== undefined) { cnt_value[5] = data.port_rights.length; }
    else { cnt_value[5] = 0; }

    html += `
    <div>
        <canvas id="my-chart"></canvas>
        <input id="my-chart-value" type="hidden" value="${cnt_value}">
    </div>
    `;
    return html;
}